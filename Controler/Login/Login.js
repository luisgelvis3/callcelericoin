...
function validacionLogin() {
    ...
    "usu.estado_usuario, usu.permiso_usuario, ase.nombre_asesor, ase.apellido_asesor, ase.extension_asesor "+
    ...
    "WHERE usu.nombre_usuario = ? AND usu.contrasena_usuario = AES_ENCRYPT( ? , UNHEX(SHA2('bc12jD=U\d7MrPr',512)))", [var_nombre_usuario, var_contrasena_usuario], function (error, result) {
        if (error) {
            ...
            try {
                if (result[0].nombre_usuario === var_nombre_usuario && result[0].contrasena_usuario === var_contrasena_usuario && result[0].permiso_usuario === "Habilitado" ) {
                    ...
                    alert('Usuario Inhabilitado pongase en contacto con el area de programacion si desea hablitar nuevamente el usuario');
                }
            } catch (error) {
                console.log(error);
                alert('Usuario Inhabilitado pongase en contacto con el area de programacion si desea hablitar nuevamente el usuario');
